(function ($) {
    ...
    var Unslider = function () {
        ...
        this.setup = function () {
            ...
            let index = this.current_slide.index() + 1;
            console.log("Step " + index);
            window.dataLayer.push({'event': 'step_' + index});
        };
        ...
        this.move = function (selector_page_to, animation, is_history_entry) {
            ...
            if (!this.slider_wrapper.is(':animated')) {
                ...
                if (selector_page_to == ('#' + _this.current_slide.attr('id'))) {
                    ...
                    // console.log("Step " + ($page_to.index() + 1));

                    let index = $page_to.index() + 1;
                    console.log("Step " + index);
                    window.dataLayer.push({'event': 'step_' + index});
                    ...
                    if (animation == 'left') {
                        this.slider_wrapper.animate({'right': '0'}, {
                            'duration': 400, 'complete': function (data) {
                        ...
                        this.slider_wrapper.animate({'left': '-100%'}, {
                            'duration': 400, 'complete': function (data) {
                                _this.slider_wrapper.css({'left': '0', 'width': '100%'});
                                _this.current_slide.hide().css({'width': '100%'});
                                _this.target_slide.css({'width': '100%'});
